...
def impact_of_road_budget_pmedian(sn, n_sample, budgets, potential='job'):
    # set params
    ins_name = 'trt'
    budget_sig = 0
    # load data
    ...
    feature, _ = load_file('./prob/trt/emb/emb.pkl')
...
def impact_of_road_budget_pmedian_ratio(sn, n_sample, budgets, potential='job'):
    # set params
    ins_name = 'trt'
    budget_sig = 0
    # load data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, help='ratio or conn')
    args = parser.parse_args()
    # run experiment
    if args.mode == 'ratio':
        for sn in args.sns:
            impact_of_road_budget_pmedian_ratio(sn, args.n, args.budgets, potential=args.potential)
    else:
        for sn in args.sns:
            impact_of_road_budget_pmedian(sn, args.n, args.budgets, potential=args.potential)
