...
class MipSolver(AbstractSolver):
    ...
    def solve(self, args, budget_proj, budget_sig, time_limit=None):
        print('\nreading the problem ...')
        ...
        print('compiling ...')
        tick = time.time()
        mip = self._construct_mip(od_pairs=od_pairs, pop=pop, budget_proj=budget_proj, budget_sig=budget_sig,
                                  proj_costs=proj_costs, sig_costs=sig_costs, G=G, T=T,
    ...
    def _construct_mip(self, od_pairs, pop, budget_proj, budget_sig, proj_costs, sig_costs, G, T, projs, edge2proj, travel_time, time_limit):
        dir_name = './prob/{}/models/discrete/mip.mps'.format(self.ins_name)
        if file_existence(dir_name):
            ...
            model.addConstrs(((gp.quicksum(x[orig, des, i, j] for i, j in G.in_edges(orig)) -
                               gp.quicksum(x[orig, des, i, j] for i, j in G.out_edges(orig))
                               == -z[orig, des]) for orig, des in od_pairs), name='flow_balance_origin')
            # flow conservation - destination
            model.addConstrs(((gp.quicksum(x[orig, des, i, j] for i, j in G.in_edges(des)) -
                               gp.quicksum(x[orig, des, i, j] for i, j in G.out_edges(des))
                               == z[orig, des]) for orig, des in od_pairs), name='flow_balance_destination')
            ...
            # travel time limit
            model.addConstrs(((gp.quicksum(travel_time[i, j] * x[orig, des, i, j] for i, j in G.edges())
                               <= T * z[orig, des]) for orig, des in od_pairs), name='travel_time_limit')
